...
export interface PacketInput {
  ...
  adverseActionNotices?: StoredAdverseActionNotice[];
}
...
export function buildPacketHtml({
  ...
  const noticeBlock = adverseActionNotices.length
    ? adverseActionNotices
        .map((notice) =>
          section(
            ...
            `<p class="muted">${escapeHtml(notice.letterSubject)} · ${formatDate(
              notice.sentAt
            )} · copy ${escapeHtml(notice.copyVersion)}</p>
            <pre style="white-space:pre-wrap;font:inherit">${escapeHtml(notice.letterText)}</pre>
            <p class="muted">Enclosed: ${escapeHtml(CFPB_SUMMARY_OF_RIGHTS_TITLE)}</p>`
          )
        )
        .join("")
    : "";
